...
from typing import Sequence, Dict, Optional, Tuple, Sequence, Union, Any
...
def _bootstrap_resample(stats: Tuple[Sequence[Sequence[Counts]],Sequence[Sequence[Counts]]],
                        metric: Metric, n_samples: int = 1000) -> Tuple[str, Sequence[Score]]:
    ...
    idxs = rng.choice(len(stats[0]), size=(n_samples, len(stats[0])), replace=True)
    ...
    scores = [
        metric._compute_score_from_stats(_s) for _s in zip(itemgetter(*idxs)(stats[0]), itemgetter(*idxs)(stats[1]))]
...
def _paired_ar_test(baseline_info: Dict[str, Tuple[np.ndarray, Result]],
    ...
    for name, metric in metrics.items():
        ...
        logging.info(f'Computing {name} for {sys_name!r} and extracting sufficient statistics')
        ...
        logging.info(f' > Performing approximate randomization test (# trials: {n_samples})')
        ...
        if n_ar_confidence > 0:
            logging.info(f' > Performing bootstrap resampling for confidence interval (# resamples: {n_ar_confidence})')
...
def _paired_bs_test(baseline_info: Dict[str, Tuple[np.ndarray, Result]],
    ...
    for name, metric in metrics.items():
        ...
        logging.info(f' > Performing paired bootstrap resampling test (# resamples: {n_samples})')
        scores_bl = np.array(
...
class PairedTest:
    ...
    def __init__(self, named_systems: Sequence[Tuple[str, Sequence[str]]],
                 metrics: Dict[str, Metric],
                 references: Optional[Sequence[Sequence[str]]],
                 test_type: str = 'ar',
                 n_samples: int = 0,
        ...
        if IS_WINDOWS:
            logging.warning('Parallel tests are not supported on Windows.')
            self.n_jobs = 1
        elif self.n_jobs == 0:
        ...
        for name, metric in metrics.items():
            logging.info(f'Pre-computing {name} statistics for {bl_name!r}')
    ...
    def __call__(self) -> Tuple[Dict[str, Signature], Dict[str, Sequence[Union[str, Result]]]]:
        """Runs the paired test either on single or multiple worker processes."""
        tasks = []
        scores: Dict[str, Sequence[Union[str, Result]]] = {}
        ...
        if self.n_jobs == 1:
            ...
            logging.info(f'Launching {self.n_jobs} parallel workers.')
